...
// demo/tracker/index.ts
import * as H from "../../dist/human.esm.js";

// demo/tracker/tracker.js
...
var require_rng_browser = __commonJS({
  "node_modules/.pnpm/uuid@3.2.1/node_modules/uuid/lib/rng-browser.js"(exports, module) {
    ...
    if (getRandomValues) {
      ...
      module.exports = __name(function whatwgRNG() {
        getRandomValues(rnds8);
        return rnds8;
      }, "whatwgRNG");
    } else {
      rnds = new Array(16);
      module.exports = __name(function mathRNG() {
...
var require_utils = __commonJS({
  "utils.js"(exports) {
    ...
    var isInsideArea = __name((area, point) => {
    ...
    var getRectangleEdges = __name((item) => {
      return {
        x0: item.x - item.w / 2,
        y0: item.y - item.h / 2,
        x1: item.x + item.w / 2,
  ...
  "node_modules/.pnpm/munkres-js@1.2.2/node_modules/munkres-js/munkres.js"(exports, module) {
    ...
    function make_cost_matrix(profit_matrix, inversion_function) {
      ...
      if (!inversion_function) {
        ...
        inversion_function = __name(function(x) {
          return maximum - x;
        }, "inversion_function");
      }
      var cost_matrix = [];
...
var require_tracker = __commonJS({
  "tracker.js"(exports) {
    ...
    var iouDistance = __name(function(item1, item2) {
...
var tracker_default = require_tracker();
...
async function detectionLoop() {
  if (!dom.video.paused && dom.video.readyState >= 2) {
    ...
    await human.detect(dom.video, humanConfig);
...
function drawLoop() {
  if (!dom.video.paused && dom.video.readyState >= 2) {
    const res = dom.interpolation.checked ? human.next(human.result) : human.result;
    let tracking = [];
    if (human.config.face.enabled)
      tracking = res.face;
    else if (human.config.body.enabled)
      tracking = res.body;
    else if (human.config.object.enabled)
      tracking = res.object;
    else
    ...
    if (dom.tracker.checked) {
      ...
      tracker_default.updateTrackedItemsWithNewFrame(items, fps.frames);
      data = tracker_default.getJSONOfTrackedItems(true);
    }
...
function initInput() {
  ...
  document.body.addEventListener("drop", async (evt) => {
    ...
    const file = (_b = (_a = evt.dataTransfer) == null ? void 0 : _a.files) == null ? void 0 : _b[0];
    if (file)
      await handleVideo(file);
    log(dom.video.readyState);
  });
  document.getElementById("inputvideo").onchange = async (evt) => {
    var _a, _b;
    evt.preventDefault();
    const file = (_b = (_a = evt.target) == null ? void 0 : _a["files"]) == null ? void 0 : _b[0];
    if (file)
      await handleVideo(file);
  };
  dom.config.onchange = () => {
    trackerConfig.distanceLimit = document.getElementById("distanceLimit").valueAsNumber;
    trackerConfig.iouLimit = document.getElementById("iouLimit").valueAsNumber;
    trackerConfig.unMatchedFramesTolerance = document.getElementById("unMatchedFramesTolerance").valueAsNumber;
    trackerConfig.unMatchedFramesTolerance = document.getElementById("unMatchedFramesTolerance").valueAsNumber;
    trackerConfig.matchingAlgorithm = document.getElementById("matchingAlgorithm-kdTree").checked ? "kdTree" : "munkres";
    tracker_default.setParams(trackerConfig);
    if (document.getElementById("keepInMemory").checked)
      tracker_default.enableKeepInMemory();
    else
      tracker_default.disableKeepInMemory();
    tracker_default.reset();
    log("tracker config change", JSON.stringify(trackerConfig));
    humanConfig.face.enabled = document.getElementById("box-face").checked;
    humanConfig.body.enabled = document.getElementById("box-body").checked;
    humanConfig.object.enabled = document.getElementById("box-object").checked;
  };
  dom.tracker.onchange = (evt) => {
    log("tracker", evt.target.checked ? "enabled" : "disabled");
    tracker_default.setParams(trackerConfig);
    tracker_default.reset();
  };
}
